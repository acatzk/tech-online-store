import React from 'react'

interface ILogo {
  className?: string
}

export const Logo: React.FC<ILogo> = ({ className }) => {
  return (
    <svg
      className={className}
      viewBox="0 0 46 41"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M22.7474 8.28613L13.6902 14.1796V21.4331L22.7474 27.3266L30.0131 22.7932V24.9465L22.7474 29.7067L13.6902 23.6998V26.3066L22.7474 32.0867L31.8047 26.3066V19.0531L24.539 23.6998V21.4331L31.8047 16.673V14.1796L22.7474 8.28613Z"
        fill="currentColor"
      />
    </svg>
  )
}

export const Menu: React.FC<ILogo> = ({ className }) => {
  return (
    <svg
      className={className}
      viewBox="0 0 26 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M4 4H21"
        stroke="currentColor"
        strokeWidth="2.4"
        strokeLinecap="round"
      />
      <path
        d="M4 10H21"
        stroke="currentColor"
        strokeWidth="2.4"
        strokeLinecap="round"
      />
      <path
        d="M4 16H21"
        stroke="currentColor"
        strokeWidth="2.4"
        strokeLinecap="round"
      />
    </svg>
  )
}

export const Cart: React.FC<ILogo> = ({ className }) => {
  return (
    <svg
      className={className}
      viewBox="0 0 21 19"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M12.8771 18.2135C13.4786 18.2135 14.0555 17.9746 14.4809 17.5492C14.9062 17.1238 15.1452 16.5469 15.1452 15.9454C15.1452 15.3438 14.9062 14.7669 14.4809 14.3416C14.0555 13.9162 13.4786 13.6772 12.8771 13.6772C12.2755 13.6772 11.6986 13.9162 11.2732 14.3416C10.8479 14.7669 10.6089 15.3438 10.6089 15.9454C10.6089 16.5469 10.8479 17.1238 11.2732 17.5492C11.6986 17.9746 12.2755 18.2135 12.8771 18.2135ZM4.93855 18.2135C5.5401 18.2135 6.11701 17.9746 6.54237 17.5492C6.96773 17.1238 7.2067 16.5469 7.2067 15.9454C7.2067 15.3438 6.96773 14.7669 6.54237 14.3416C6.11701 13.9162 5.5401 13.6772 4.93855 13.6772C4.337 13.6772 3.76009 13.9162 3.33473 14.3416C2.90937 14.7669 2.67041 15.3438 2.67041 15.9454C2.67041 16.5469 2.90937 17.1238 3.33473 17.5492C3.76009 17.9746 4.337 18.2135 4.93855 18.2135ZM19.7246 2.24918C20.0077 2.24005 20.2762 2.12117 20.4732 1.91767C20.6702 1.71418 20.7804 1.44203 20.7804 1.15877C20.7804 0.875515 20.6702 0.603363 20.4732 0.399868C20.2762 0.196374 20.0077 0.0774907 19.7246 0.0683594H18.4193C17.3963 0.0683594 16.5118 0.778289 16.2895 1.77627L14.8685 8.17471C14.6462 9.17269 13.7616 9.88262 12.7387 9.88262H4.21955L2.58422 3.33902H13.1821C13.4626 3.32623 13.7273 3.20581 13.9212 3.00283C14.1151 2.79985 14.2234 2.52992 14.2234 2.24918C14.2234 1.96844 14.1151 1.69851 13.9212 1.49553C13.7273 1.29255 13.4626 1.17213 13.1821 1.15934H2.58422C2.25269 1.15924 1.9255 1.23473 1.62753 1.38007C1.32956 1.5254 1.06864 1.73676 0.86462 1.99808C0.660597 2.25939 0.518837 2.56379 0.450117 2.88811C0.381397 3.21244 0.387527 3.54817 0.468039 3.86977L2.10337 10.4111C2.22126 10.8831 2.49359 11.3021 2.87704 11.6015C3.2605 11.9009 3.73305 12.0635 4.21955 12.0634H12.7387C13.7312 12.0636 14.6942 11.7252 15.4684 11.1041C16.2426 10.4831 16.7819 9.61654 16.9971 8.64762L18.4193 2.24918H19.7246Z"
        fill="currentColor"
      />
    </svg>
  )
}

export const User: React.FC<ILogo> = ({ className }) => {
  return (
    <svg
      className={className}
      viewBox="0 0 12 12"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M6 0C6.79565 0 7.55871 0.31607 8.12132 0.87868C8.68393 1.44129 9 2.20435 9 3C9 3.79565 8.68393 4.55871 8.12132 5.12132C7.55871 5.68393 6.79565 6 6 6C5.20435 6 4.44129 5.68393 3.87868 5.12132C3.31607 4.55871 3 3.79565 3 3C3 2.20435 3.31607 1.44129 3.87868 0.87868C4.44129 0.31607 5.20435 0 6 0ZM6 7.5C9.315 7.5 12 8.8425 12 10.5V12H0V10.5C0 8.8425 2.685 7.5 6 7.5Z"
        fill="currentColor"
      />
    </svg>
  )
}

export const Clock: React.FC<ILogo> = ({ className }) => {
  return (
    <svg
      className={className}
      viewBox="0 0 30 30"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M15 24.5642C9.72753 24.5642 5.43571 20.2724 5.43571 14.9999C5.43571 9.72747 9.72753 5.43564 15 5.43564C20.2725 5.43564 24.5643 9.72747 24.5643 14.9999C24.5643 20.2724 20.2725 24.5642 15 24.5642Z"
        stroke="currentColor"
        strokeWidth="2.3"
      />
      <path
        d="M15 10.5V15.75H20.25"
        stroke="currentColor"
        strokeWidth="2.3"
        strokeLinecap="round"
      />
    </svg>
  )
}

export const Location: React.FC<ILogo> = ({ className }) => {
  return (
    <svg
      className={className}
      viewBox="0 0 18 21"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M8.57143 10.5714C7.39286 10.5714 6.42857 9.64564 6.42857 8.51422C6.42857 7.38279 7.39286 6.45707 8.57143 6.45707C9.75 6.45707 10.7143 7.38279 10.7143 8.51422C10.7143 9.64564 9.75 10.5714 8.57143 10.5714ZM15 8.71993C15 4.98622 12.1607 2.34279 8.57143 2.34279C4.98214 2.34279 2.14286 4.98622 2.14286 8.71993C2.14286 11.1268 4.23214 14.3154 8.57143 18.1211C12.9107 14.3154 15 11.1268 15 8.71993ZM8.57143 0.285645C13.0714 0.285645 17.1429 3.59764 17.1429 8.71993C17.1429 12.1348 14.2821 16.1771 8.57143 20.8571C2.86071 16.1771 0 12.1348 0 8.71993C0 3.59764 4.07143 0.285645 8.57143 0.285645Z"
        fill="currentColor"
      />
    </svg>
  )
}

export const LogoWide: React.FC<ILogo> = ({ className }) => {
  return (
    <svg
      className={className}
      viewBox="0 0 29 34"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M14.1101 0.52002L0.130463 8.71339V18.7975L14.1101 26.9909L25.3245 20.6883V23.6821L14.1101 30.2998L0.130463 21.9488V25.5728L14.1101 33.6086L28.0897 25.5728V15.4887L16.8753 21.9488V18.7975L28.0897 12.1798V8.71339L14.1101 0.52002Z"
        fill="currentColor"
      />
    </svg>
  )
}
